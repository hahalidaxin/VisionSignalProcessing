...
class BMPReader :
    def __init__(self, filePath) :
        ...
        for row in range(self.biHeight) :
            ...
            self.G.append(G_row)
            self.R.append(R_row)
        self.B = np.array(self.B, dtype = np.uint8)
        self.G = np.array(self.G, dtype = np.uint8)
        self.R = np.array(self.R, dtype = np.uint8)
...
if __name__ == "__main__":
    ...
    bmp = BMPReader(img_path).getImg()
    
    cv2.imwrite('bmp.png', bmp)
    # cv2.imshow("bmp", bmp)
